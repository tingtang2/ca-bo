...
class InfoTransformerVAE(pl.LightningModule):
    ...
    def generate_pad_mask(self, tokens):
        ...
        inds = mask.float().argmax(
            dim=-1)  # Returns first index along axis when multiple present
    ...
    def encode(self, tokens, as_probs=False):
        ...
        sigma = F.softplus(encoding[...,
                                    self.d_model:]) + self.min_posterior_std
    ...
    def decode(self, z, tokens, as_probs=False):
        ...
        embed = torch.cat(
            [
                ...
                torch.zeros(
                    embed.shape[0], 1, embed.shape[-1], device=self.device),
                embed
            ],
            dim=1)
    ...
    def sample(self,
        ...
        tokens = torch.zeros(
            n, 1,
            device=self.device).long()  # Start token is 0, stop token is 1
        ...
        while True:  # Loop until every molecule hits a stop token
            ...
            logits = decoding @ self.decoder_token_unembedding
            sample, randoms = gumbel_softmax(logits,
                                             dim=-1,
                                             hard=True,
                                             return_randoms=True)
    ...
    @torch.no_grad()
    def greedy_decode(self, z: Tensor):
        model_state = self.training
        self.eval()
        n = z.shape[0]
        ...
        while True:  # Loop until every molecule hits a stop token
            ...
            logits = decoding @ self.decoder_token_unembedding
            tokens = torch.cat(
                [tokens, logits[:, -1, :].argmax(dim=-1)[:, None]], dim=-1)
    ...
    def forward(self, tokens):
        ...
        recon_loss = F.cross_entropy(
            logits.permute(0, 2, 1), tokens,
            reduction='none').mean()  # .sum(1).mean(0)
        ...
        sigma2 = sigma.pow(2)
        kldiv = 0.5 * (mu.pow(2) + sigma2 - sigma2.log() -
                       1).mean()  # .sum(dim=(1, 2)).mean(0)
        ...
        return dict(
            ...
            recon_string_acc=(logits.argmax(dim=-1) == tokens).all(
                dim=1).float().mean(dim=0),
...
class VAEModule(pl.LightningModule):
    ...
    def on_train_epoch_end(self):
        if self.trainer.is_global_zero:
            ...
            self.log('train_frac_valid_epoch_end',
                     is_valid.mean(),
                     rank_zero_only=True)
        ...
        def decoder_lr_sched(step):
            if step < ENCODER_WARMUP_STEPS:
                ...
                if (step - ENCODER_WARMUP_STEPS + 1) % AGGRESSIVE_STEPS == 0:
                    return min((step - ENCODER_WARMUP_STEPS) /
                               (DECODER_WARMUP_STEPS * AGGRESSIVE_STEPS), 1.)
                else:
                    return 0.

        optimizer = Adam([
            dict(params=encoder_params, lr=ENCODER_LR),
            dict(params=decoder_params, lr=DECODER_LR)
        ])
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, [encoder_lr_sched, decoder_lr_sched])

        return dict(optimizer=optimizer,
                    lr_scheduler=dict(scheduler=lr_scheduler,
                                      interval='step',
                                      frequency=1))
...
def load(dataset_path, checkpoint_path):
    ...
    module = VAEModule.load_from_checkpoint(checkpoint_path,
...
def fit():
    ...
    check = ModelCheckpoint(every_n_epochs=10, save_top_k=-1, save_last=True)
    trainer = pl.Trainer(
        gpus=-1,
