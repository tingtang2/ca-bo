...
KWARG_ERR_MSG = ("`MCSampler`s no longer support the `{}` argument. "
...
class MCSampler(Module, ABC):
    ...
    def __init__(
        ...
        self.seed = seed if seed is not None else torch.randint(
            0, 1000000, (1, )).item()
    ...
    def _get_extended_base_sample_shape(self,
                                        posterior: Posterior) -> torch.Size:
    ...
    def _update_base_samples(self, posterior: Posterior,
                             base_sampler: MCSampler) -> None:
