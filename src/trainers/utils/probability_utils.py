...
def get_constants(
    ...
    return tuple(
        torch.full((), val, dtype=dtype, device=device) for val in values)
...
def gen_positional_indices(
    ...
    for i, s in enumerate(reversed(shape[:_dim + 1])):
        yield torch.arange(0, s * cumsize, cumsize,
                           device=device)[(..., ) + i * (None, )]
        cumsize *= s
...
def swap_along_dim_(
    ...
    if dim and ((isinstance(i, Tensor) and i.ndim) or
                (isinstance(j, Tensor) and j.ndim)):
        ...
        pidx = build_positional_indices(shape=values.shape[:dim + 1],
                                        dim=-2,
        ...
        if isinstance(i, Tensor) and i.ndim > 1:
            raise ValueError(
                "Tensor `i` must be at most 1-dimensional when `dim=0`.")

        if isinstance(j, Tensor) and j.ndim > 1:
            raise ValueError(
                "Tensor `j` must be at most 1-dimensional when `dim=0`.")
