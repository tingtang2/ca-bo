...
class InducingPointAllocator(ABC):
    ...
    @abstractmethod
    def _get_quality_function(self, ) -> QualityFunction:
        """
    ...
    def allocate_inducing_points(
        ...
        # multi-task case
        elif train_train_kernel.ndimension() == 3 and len(
                input_batch_shape) == 0:
            quality_scores = quality_function(inputs)
        ...
        # batched input cases
            ...
            batched_inputs = (inputs.expand(*input_batch_shape, -1, -1)
                              if inputs.ndimension() == 2 else inputs)
            ...
            for input_element in reshaped_inputs:
                ...
                kernel_element = (kernel_element[0]
                                  if kernel_element.ndimension() == 3 else
                                  kernel_element)
                quality_scores = quality_function(input_element)
...
class ExpectedImprovementQualityFunction(QualityFunction):
    ...
    def __call__(self, inputs: Tensor) -> Tensor:  # [n, d] -> [n]
        ...
        u = (mean -
             best_f) / sigma if self._maximize else -(mean - best_f) / sigma
