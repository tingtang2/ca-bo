...
class CaGPSlidingWindowTrainer(CaGPTrainer):
    ...
    def run_experiment(self, iteration: int):
        ...
        if self.freeze_actions:
            self.model.actions_op.blocks.data = torch.ones((train_x.shape[0], self.model.num_non_zero))
        ...
        for i in trange(self.max_oracle_calls - self.num_initial_points):
            ...
            if i > 0:
                ...
                if self.add_actions_by_reinit and train_x.size(0) <= proj_dim:
                    ...
                    train_targets, train_Yvar = self.model.outcome_transform(Y=update_y[0:self.model.num_non_zero * self.model.projection_dim].unsqueeze(1),
                                                                             Yvar=None,
                    ...
                    if train_x.size(0) <= proj_dim:
                        ...
                        if self.freeze_actions:
                            ...
                            if self.non_zero_action_init:
                                ...
                                cast_threshold = 1
                                new_action = torch.where((new_action > 0) & (new_action < cast_threshold),
                                                         cast_threshold,
                                                         new_action)
                                ...
                                self.model.actions_op.blocks.data = torch.concat((self.model.actions_op.blocks.data[1:],
                                                                                  new_action))
                            else:
                        ...
                        if self.reinit_hyperparams:
                            self.model.likelihood = GaussianLikelihood().to(self.device)
                            base_kernel = gpytorch.kernels.MaternKernel(2.5, ard_num_dims=None)
            ...
            if self.optimizer_type == 'botorch_lbfgs':
                self.model.train()
                mll = ComputationAwareELBO(self.model.likelihood, self.model, return_elbo_terms=False)
                mll = fit_gpytorch_mll(mll)
                epochs_trained = -1
                final_loss = -1
            else:
                mll = ComputationAwareELBO(self.model.likelihood, self.model, return_elbo_terms=True)
            ...
            if self.freeze_actions:
                assert (self.model.actions_op.blocks.data == torch.ones((update_x.size(0), 1))).all()
...
class Med2LogEICaGPSlidingWindowTrainer(CaGPSlidingWindowTrainer, GuacamolTrainer, LogEITrainer):
